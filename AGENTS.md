...
- **Database-enforced consent** (RLS in 0002): you always read your own time entries; you read another member's *detail* only if their sharing level is `full`; `totals`/`private` detail is hidden (a SECURITY DEFINER `time_entry_totals` view exposes aggregates for `totals` members). Writes are owner-only **except a manager may administer any entry they can see** (so the Founder can mark others' billable time as billed for invoicing — still bounded by the read policy, so never a `private`/`totals` member's detail). Rates are manager-only; sharing levels are self-only. `current_member_id()` / `is_manager()` are SECURITY DEFINER helpers that resolve the caller from their JWT email without recursing on `members`.
